...
const ITEMS = ['Dean', 'Lil B', 'Lazer', 'Simz', 'Bladee', 'Julius'];
          ...
          {ITEMS.map((_, index) => {
            return (
              <motion.div
                key={index}
                initial={false}
                className='h-[500px] w-[200px] rounded-[12px] bg-stone-200/60 transition-all duration-150 dark:bg-stone-800'
                style={{
                  y: currentIndex === index ? 0 : -12,
                }}
                transition={transition}
              />
            );
